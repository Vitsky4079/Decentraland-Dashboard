...
const COLORS = {
  owned: 'rgba(255,188,91,0.55)',    // --peach
  district: 'rgba(124,140,248,0.55)', // --info
  plaza: 'rgba(63,203,110,0.55)',    // --ok
  road: 'rgba(120,113,140,0.55)',    // muted neutral
};
const BORDER = 'rgba(8,7,12,0.65)'; // --bg, used as the grout line between parcels

async function fetchParcels(minX, maxX, minY, maxY) {
  const url =
    `${SUPABASE_URL}/rest/v1/land_parcels?select=x,y,type,edge_top,edge_left` +
...
module.exports = async (req, res) => {
  ...
  if (minX <= maxX && minY <= maxY) {
    try {
      ...
      for (const p of parcels) {
        ...
        const fill = COLORS[p.type] || COLORS.owned;
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${SIDE}" height="${SIDE}" viewBox="0 0 ${SIDE} ${SIDE}">${rects}</svg>`;
